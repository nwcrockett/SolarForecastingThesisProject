...
def split_dataset(data):
    # split data into a test and train set, 322 rows of test data
...
def to_supervised(train, n_input, n_out=7):
    # flatten data back into a continous list data vectors no longer seperated into sets
    data = train.reshape((train.shape[0]*train.shape[1], train.shape[2]))
    X, y = list(), list()
    in_start = 0  # start point of taking data
    # step over the entire history one time step at a time
    for _ in range(len(data)):
        # define the end of the input sequence
        in_end = in_start + n_input  # Input for y
        out_end = in_end + n_out  # expected output for step of data
